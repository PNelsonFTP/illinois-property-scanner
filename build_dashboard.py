#!/usr/bin/env python3
"""Regenerate dashboard HTML from v2_compiled.json"""
...
VERIFIED_NOTE = "Live-verified via Realtor.com MLS status. Pending/contingent listings excluded."
...
def main():
    ...
    verified = sum(1 for p in data if p.get("verification_source") == "realtor.com-live")
...
.hdr-meta{{display:flex;gap:14px;align-items:center;font-size:.75rem;color:var(--text2)}}
...
<div class="hdr"><div class="hdr-in">
  <div class="hdr-top">
    ...
    <div class="hdr-meta">
      <span>Scan: {ts}</span>
      <strong>{len(data)} properties</strong>
      <span class="scope-badge">~3 mi radius</span>
      <span class="verify-badge">{verified} live-verified</span>
    </div>
  </div>
  <div class="badges" id="hBdg"></div>
</div></div>
<div class="wrap">
  <div class="src-note">{VERIFIED_NOTE} Run <code>python scan.py</code> to refresh. Scope: Wheaton, Oswego, Sandwich, Somonauk, Lake Holiday/Sheridan IL.</div>
  <div class="ctrls">
    <div class="cg"><label>Town</label><select id="fA" onchange="go()"><option value="">All</option><option>Wheaton</option><option>Oswego</option><option>Sandwich</option><option>Somonauk</option><option>Lake Holiday</option></select></div>
    ...
    <div class="cg"><label>Distress</label><select id="fD" onchange="go()"><option value="">All</option><option value="foreclosure">Foreclosure</option><option value="as-is">As-Is/Fixer</option><option value="price-reduced">Price Reduced</option><option value="high-dom">High DOM</option><option value="below-market">Below Market</option></select></div>
    <div class="cg"><label>Verified Only</label><select id="fV" onchange="go()"><option value="">All</option><option value="live">Live-Verified Only</option></select></div>
    <div class="cg"><label>Max Price</label><input type="number" id="fP" placeholder="No max" onchange="go()"></div>
    <div class="cg"><label>Min Score</label><select id="fS" onchange="go()"><option value="0">All</option><option value="3">3+</option><option value="4">4+</option><option value="5">5+</option></select></div>
    <div class="cg"><label>Sort</label><select id="fO" onchange="go()"><option value="score">Score</option><option value="dom">DOM</option><option value="price-asc">Price Low</option><option value="price-desc">Price High</option></select></div>
  ...
  <div class="stats">
    ...
    <div class="sc"><h3>Distress Types</h3><table class="st" id="sD"></table></div>
    <div class="sc"><h3>Property Types</h3><table class="st" id="sT"></table></div>
  </div>
  <div class="tp"><h3>Top Picks</h3><div id="tpL"></div></div>
...
let h='';AS.forEach(a=>{{h+=`<tr><td>${{a.area}}</td><td>${{a.count}}</td><td>${{a.avgPrice}}</td><td>${{a.avgDom}} DOM</td></tr>`}});$('sA').innerHTML=h;
h='';B.forEach(b=>{{h+=`<tr><td>${{b.tag}}</td><td>${{b.count}}</td></tr>`}});$('sD').innerHTML=h;
h='';const ty={{}};P.forEach(p=>{{ty[p.property_type]=(ty[p.property_type]||0)+1}});Object.entries(ty).sort((a,b)=>b[1]-a[1]).forEach(([k,v])=>{{h+=`<tr><td>${{k}}</td><td>${{v}}</td></tr>`}});$('sT').innerHTML=h;
function go(){{
  let f=[...P];
  const a=$('fA').value,t=$('fT').value,d=$('fD').value,v=$('fV').value,mp=parseFloat($('fP').value)||Infinity,ms=parseInt($('fS').value)||0,o=$('fO').value,q=$('fQ').value.toLowerCase().trim();
  if(a)f=f.filter(p=>p.nearest_target===a);
  if(t)f=f.filter(p=>p.property_type===t);
  if(v==='live')f=f.filter(p=>p.verification_source==='realtor.com-live');
  ...
  if(q)f=f.filter(p=>(p.address+' '+(p.notes||'')+' '+p.distress_types.join(' ')).toLowerCase().includes(q));
  f.sort((a,b)=>{{switch(o){{case'score':return(b.distress_score-a.distress_score)||(b.dom||0)-(a.dom||0);case'price-asc':return(a.list_price||1e9)-(b.list_price||1e9);case'price-desc':return(b.list_price||0)-(a.list_price||0);case'dom':return(b.dom||0)-(a.dom||0);default:return 0}}}});
  $('rc').innerHTML=`<strong>${{f.length}}</strong> of ${{P.length}} properties`;
  ...
  $('grd').innerHTML=f.map(p=>{{
    const verified=p.verification_source==='realtor.com-live';
    ...
    return`<div class="cd" onclick="this.classList.toggle('open')"><div class="cd-s ${{tcl(p.distress_score)}}">${{p.distress_score}}</div>${{img}}<div class="cd-b">
      ...
      <div class="cd-x"><div class="xg">
        ...
        <div class="xi"><span class="l">Verified:</span> <span class="v ${{verified?'v-ok':'v-warn'}}">${{verified?'Live MLS':'Legacy'}}</span></div>
        <div class="xi"><span class="l">Score:</span> <span class="v">${{p.distress_score}}</span></div>
...
go();
    ...
    OUT.write_text(html)
    print(f"Wrote dashboard: {OUT} ({len(data)} properties, {verified} live-verified)")
